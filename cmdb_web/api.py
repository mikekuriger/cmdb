...
SELECT
    ...
    n.first_seen, n.last_seen, n.active, n.is_template,
...
def update_node(node_id):
    ...
    existing = query(f'SELECT {cols} FROM nodes WHERE id=%s', (node_id,), one=True)
    if existing:
        for field, new_val in updates.items():
            old_val = existing.get(field)
            if str(old_val or '') != str(new_val or ''):
                execute(
                    'INSERT INTO node_history (node_id, field, old_value, new_value, source, changed_by) '
...
def delete_node(node_id):
    row = query('SELECT id, name FROM nodes WHERE id=%s', (node_id,), one=True)
    if not row:
        return jsonify(error='Not found'), 404
...
def recent_changes():
    ...
    rows = query(
        'SELECT h.id, h.node_id, n.name AS node_name, h.field, '
        '       h.old_value, h.new_value, h.source, h.changed_by, h.changed_at '
        'FROM node_history h '
    ...
    def s(r):
        out = dict(r)
        if out.get('changed_at') and isinstance(out['changed_at'], datetime):
            out['changed_at'] = out['changed_at'].strftime('%Y-%m-%d %H:%M:%S')
        return out
...
def groups():
    p = request.args
    conds, vals = ['1=1'], []
    if p.get('ansible') in ('1', 'true'): conds.append('g.is_ansible=1')
    if p.get('nagios')  in ('1', 'true'): conds.append('g.is_nagios=1')
    if p.get('q'):
