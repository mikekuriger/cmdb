...
def node_detail(node_id):
    ...
    groups    = query('SELECT g.id, g.name, g.type FROM groups_ g JOIN node_groups ng ON ng.group_id=g.id WHERE ng.node_id=%s ORDER BY g.name', (node_id,))
    all_groups= query('SELECT id, name, type FROM groups_ ORDER BY name')
